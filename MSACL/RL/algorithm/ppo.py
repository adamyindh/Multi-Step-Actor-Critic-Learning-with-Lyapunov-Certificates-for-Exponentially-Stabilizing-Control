...
The PPO algorithm
Reference:
This algorithm is implemented based on the work Proximal policy optimization algorithms.
...
class ApproxContainer(nn.Module):
    """PPO networks: value, Lyapunov, policy (separate optimizers)"""
...
class PPO:
    """PPO algorithm (on-policy, clip-based update)"""
    def __init__(
        self,
        *,  # Force keyword args only
        max_iteration: int,
        num_repeat: int,
        num_mini_batch: int,
        mini_batch_size: int,
        ...
        clip: float = 0.1,  # PPO clip coefficient
        beta: float = 0.2,  # Hybrid advantage weight
        lya_diff_scale: float = 1.0,  # Lyapunov difference term weight
        lya_zero_scale: float = 20.0,  # Lyapunov zero term weight
        lya_positive_scale: float = 1.0,  # Lyapunov positive term weight
        ...
        loss_coefficient_entropy: float = 0.01,  # Entropy loss weight (exploration)
        ...
        **kwargs
        ...
        # Batch size (scale with env number)
    ...
    def model_update(self, data: Dict[str, Tensor]):
        """PPO core update pipeline (preprocess -> repeat mini-batch training)"""
        start_time = time.perf_counter()

        # Env-specific state correction (HalfCheetah-v4 relative state)
        ...
        # PPO repeat training (on-policy data reuse)
        for _ in range(self.num_repeat):
            ...
            for n in range(self.num_mini_batch):
                ...
                # Adam lr annealing (linear schedule)
    ...
    def _compute_loss_lya(self, data: Dict[str, Tensor]):
        """Compute Lyapunov Risk loss (zero + difference term)"""
        obs, obs2 = data["obs"], data["obs2"]

        # Lyapunov zero term (origin value = 0)
        if self.env_id == "HalfCheetah-v4":
        ...
        # Lyapunov difference term (diff ≤ 0)
    ...
    def _compute_loss_value(self, data: Dict[str, Tensor]):
        ...
        # Value loss (optional normalization)
